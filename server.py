...
SLOPE_FOLDER = parser.get('server', 'slope-folder')
ASPECT_FOLDER = parser.get('server','aspect-folder')
SLOPE_URL =  parser.get('server', 'slope')
ASPECT_URL =  parser.get('server', 'aspect')
...
slope = GDALTileInterface(SLOPE_FOLDER, '%s/summary.json' % SLOPE_FOLDER, OPEN_INTERFACES_SIZE)
aspect = GDALTileInterface(ASPECT_FOLDER, '%s/summary.json' % ASPECT_FOLDER, OPEN_INTERFACES_SIZE)
...
if slope.has_summary_json() and not ALWAYS_REBUILD_SUMMARY:
    print('Re-using existing summary JSON')
    slope.read_summary_json()
else:
    print('Creating summary JSON ...')
    slope.create_summary_json()


if aspect.has_summary_json() and not ALWAYS_REBUILD_SUMMARY:
    print('Re-using existing summary JSON')
    aspect.read_summary_json()
else:
    print('Creating summary JSON ...')
    aspect.create_summary_json()
...
def get_slope(lat, lng):
    """
    Get the slope at point (lat,lng) using the currently opened interface
    :param lat:
    :param lng:
    :return:
    """
...
def get_aspect(lat, lng):
    """
    Get the aspect at point (lat,lng) using the currently opened interface
    :param lat:
    :param lng:
    :return:
    """
...
def do_getSlope(get_locations_func):
    """
    Generic method which gets the locations in [(lat,lng),(lat,lng),...] format by calling get_locations_func
    and returns an answer ready to go to the client.
    :return:
    ...
    try:
        locations = get_locations_func()
        return {'results': [get_slope(lat, lng) for (lat, lng) in locations]}
    except InternalException as e:
        response.status = 400
        response.content_type = 'application/json'
        return e.args[0]
...
def do_getAspect(get_locations_func):
    """
    Generic method which gets the locations in [(lat,lng),(lat,lng),...] format by calling get_locations_func
    and returns an answer ready to go to the client.
    :return:
    ...
    try:
        locations = get_locations_func()
        return {'results': [get_aspect(lat, lng) for (lat, lng) in locations]}
    except InternalException as e:
        response.status = 400
        response.content_type = 'application/json'
        return e.args[0]
...
# For CORS
@route(SLOPE_URL, method=['OPTIONS'])
def cors_handler():
    return {}

@route(SLOPE_URL, method=['GET'])
def get_geSlope():
    """
    GET method. Uses query_to_locations.
    :return:
    """
    return do_getSlope(query_to_locations)


@route(SLOPE_URL, method=['POST'])
def post_getSlope():
    """
    GET method. Uses body_to_locations.
    :return:
    """
    return do_getSlope(body_to_locations)

# For CORS
@route(ASPECT_URL, method=['OPTIONS'])
def cors_handler():
    return {}

@route(ASPECT_URL, method=['GET'])
def get_getAspect():
    """
    GET method. Uses query_to_locations.
    :return:
    """
    return do_getAspect(query_to_locations)


@route(ASPECT_URL, method=['POST'])
def post_getAspect():
    """
    GET method. Uses body_to_locations.
    :return:
    """
    return do_getAspect(body_to_locations)
