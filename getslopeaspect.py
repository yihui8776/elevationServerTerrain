...
#计算xy方向的梯度
def CacdxdySb(npgrid,sizex,sizey):
    ...
    #sb算法
    #sdx = (z3 + 2z6 + z9 - z1 - 2z4 - z7)/8w
    #sdy = (z1 + 2z2 + z3 - z7 - 2z8 - z9)/8w
...
#计算xy方向的梯度
def CacdxdyEY(npgrid,sizex,sizey):
    ...
    # 坡度、坡向变化率的计算：https://help.arcgis.com/zh-cn/arcgisdesktop/10.0/help/index.html#/na/009z000000vz000000/
    #EY算法
    #sdx = (z3 + z6 + z9 - z1 - z4 - z7)/6w
    #sdy = (z1 + z2 + z3 - z7 - z8 - z9)/6w
    for i in range(1,nx-1):
        for j in range(1,ny-1):
            s_dx[i,j] = ((npgrid[i-1,j+1]+npgrid[i,j+1]+npgrid[i+1,j+1])-(npgrid[i-1,j-1]+npgrid[i,j-1]+npgrid[i+1,j-1])) / (6 * sizex)
            s_dy[i,j] = ((npgrid[i+1, j-1] +  npgrid[i+1, j] + npgrid[i+1,j+1])-(npgrid[i-1,j-1]+ npgrid[i-1,j] + npgrid[i-1,j+1])) / (6 * sizey)
...
#计算坡度
def CacSlope(s_dx,s_dy,a_dx,a_dy):
...
#计算坡向
def CacAsp(s_dx,s_dy,a_dx,a_dy):
...
if __name__ == "__main__":
    ...
    # 读取ASTER GDEM遥感影像
    demgrid, proj, geotrans, row, column, band, type = read_img('data/fujian125.tif')
    oridata = demgrid
    # 为计算梯度给影像添加周围一圈数据
    demgrid = AddRound(demgrid)
    demgrid = rd.rdarray(demgrid, no_data = -9999) 
    # 梯度计算
    #dx1,dy1,dx2,dy2 = Cacdxdy(demgrid,30,30)
    #dx1,dy1,dx2,dy2 = CacdxdySb(demgrid,12.5,12.5)
    # 坡度、坡向计算
    #slope,aspect = CacSlopAsp(dx1,dy1,dx2,dy2)
    #slope = CacSlope(dx1,dy1,dx2,dy2)
    slope = rd.TerrainAttribute(demgrid,attrib= 'slope_riserun')
    aspect = rd.TerrainAttribute(demgrid,attrib= 'aspect') 
    ...
    slopeT = write_img('data/slope.tif', tar_proj, geotrans, slope, type)
    del slope
    aspectT = write_img('data/aspect.tif', tar_proj, geotrans, aspect, type)
